...
ORIGIN_SAM_PATH=$SPEC_BENCH_PATH/local_cache/static_sam_origin_dict.pkl
SMALL_SAM_PATH=$SPEC_BENCH_PATH/local_cache/static_sam_small_dict_after.pkl
...
# CUDA_VISIBLE_DEVICES=${GPU_DEVICES} PYTHONPATH=$SPEC_BENCH_PATH python -m evaluation.inference_samd --model-path $Vicuna_PATH --model-id ${MODEL_NAME}-samd-origin --bench-name $bench_NAME --temperature $TEMP --dtype $torch_dtype --samd_n_predicts 40 --samd_len_threshold 5 --samd_len_bias 20 --attn_implementation sdpa --static_sam_path $ORIGIN_SAM_PATH
CUDA_VISIBLE_DEVICES=${GPU_DEVICES} PYTHONPATH=$SPEC_BENCH_PATH python -m evaluation.inference_samd --model-path $Vicuna_PATH --model-id ${MODEL_NAME}-samd-small-after --bench-name $bench_NAME --temperature $TEMP --dtype $torch_dtype --samd_n_predicts 40 --samd_len_threshold 5 --samd_len_bias 20 --attn_implementation sdpa --static_sam_path $SMALL_SAM_PATH
